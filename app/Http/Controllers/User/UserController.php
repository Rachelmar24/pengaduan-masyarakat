...
class UserController extends Controller
{
    ...
    public function register(Request $request)
    {
        ...
        // $link = URL::temporarySignedRoute('bogor.verify', now()->addMinutes(30), ['nik' => $data['nik']]);
        // Mail::to($data['email'])->send(new VerifikasiEmailUntukRegistrasiPengaduanMasyarakat($data['nama'], $link));

        // Arahkan ke route bogor.index
        return redirect()->route('bogor.index');
    }
    ...
    public function storePengaduan(Request $request)
    {
        ...
        if ($pengaduan) {
            ...
            return redirect()->route('bogor.laporan', 'me')->with(['pengaduan' => 'Berhasil terkirim!', 'type' => 'success']);
        } else {
